(ns syngenta.main
  "Kotodama WASM entrypoint for the Syngenta clean-room actor (L4) — Clojure port.
  ...
  facts (`syngenta.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "syngenta")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"syngenta.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "syngenta-compat" :tier tier :entities entities} 200])
...
(defn start! [] :syngenta-compat/ready)
